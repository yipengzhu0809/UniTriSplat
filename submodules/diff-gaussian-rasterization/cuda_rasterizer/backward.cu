...
__global__ void computeCov2DCUDAOmni(int P,
	const float3* means,
	const int* radii,
	...
	bool antialiasing)
{
	...
	float3 dL_dconic = { dL_dconics[4 * idx], dL_dconics[4 * idx + 1], dL_dconics[4 * idx + 3] };
	float3 t = transformPoint4x3(mean, view_matrix);
	float2 p_proj = p3d2lonlat(t);
	float lat_scale = cosf(p_proj.y); 
	lat_scale = fmaxf(lat_scale, 1e-6f);
	
	float trxztrxz = t.x * t.x + t.z * t.z; // tx^2 + tz^2
	float trxztrxz_inv = 1.0f / (trxztrxz + 0.0000001f); // 1 / (tx^2 + tz^2)
	float trxz = sqrtf(trxztrxz); // sqrt(tx^2 + tz^2)
	float trxz_inv = 1.0f / (trxz + 0.0000001f); // 1 / sqrt(tx^2 + tz^2)
	float trtr = trxztrxz + t.y * t.y; // tx^2 + ty^2 + tz^2
	float trtr_inv = 1.0f / (trtr + 0.0000001f); // 1 / (tx^2 + ty^2 + tz^2)
	float tr = sqrtf(trtr); // sqrt(tx^2 + ty^2 + tz^2)
	float tr_inv = 1.0f / (tr + 0.0000001f); // 1 / sqrt(tx^2 + ty^2 + tz^2)
	float tr3 = tr * tr * tr; // (tx^2 + ty^2 + tz^2)^(3/2)
	float tr3_inv = 1.0f / (tr3 + 0.0000001f); // 1 / (tx^2 + ty^2 + tz^2
	...
	dlon_dt[idx].x = dgx_dtx;
	dlon_dt[idx].y = dgx_dty;
	dlon_dt[idx].z = dgx_dtz;

	dlat_dt[idx].x = dgy_dtx;
	dlat_dt[idx].y = dgy_dty;
	dlat_dt[idx].z = dgy_dtz;
	...
	// Use helper variables for 2D covariance entries. More compact.
	float c_xx = clamp_cov_arc.x;
	float c_xy = clamp_cov_arc.y;
	float c_yy = clamp_cov_arc.z;
	...
	float dL_dc_xy = 0;
	float dL_dc_yy = 0;
	float dL_dlat_from_cov = 0.0f;  // Default to zero.

	float denom = c_xx * c_yy - c_xy * c_xy;
	denom = fmaxf(denom, CLAMP_NUM);
	...
	if (denom2inv != 0)
	{
		...
		// given gradients of loss w.r.t. conic matrix (inverse covariance matrix).
		// e.g., dL / da = dL / d_conic_a * d_conic_a / d_a
		//the dL_dc_xx is the dL/d(cov_arc)
		dL_dc_xx += denom2inv * (-c_yy * c_yy * dL_dconic.x + 2 * c_xy * c_yy * dL_dconic.y + (denom - c_xx * c_yy) * dL_dconic.z);
		dL_dc_yy += denom2inv * (-c_xx * c_xx * dL_dconic.z + 2 * c_xx * c_xy * dL_dconic.y + (denom - c_xx * c_yy) * dL_dconic.x);
		dL_dc_xy += denom2inv * 2 * (c_xy * c_yy * dL_dconic.x - (denom + 2 * c_xy * c_xy) * dL_dconic.y + c_xx * c_xy * dL_dconic.z);
		
		float cov_rad_xx = cov2D[0][0];
		float cov_rad_xy = cov2D[0][1];

		float dL_dlat_scale = dL_dc_xx * 2.0f * cov_rad_xx * lat_scale + dL_dc_xy * cov_rad_xy;
		float lat = p_proj.y;
		float sin_lat = sinf(lat);
		dL_dlat_from_cov = dL_dlat_scale * (-sin_lat);

		//convert back to dL/d(cov_rad)
		dL_dc_xx *= lat_scale * lat_scale;
		dL_dc_xy *= lat_scale;
		// Gradients of loss L w.r.t. each 3D covariance matrix (Vrk) entry,
		// given gradients w.r.t. 2D covariance matrix (diagonal).
		// cov2D = transpose(T) * transpose(Vrk) * T;
	...
	float dJ_dx00 = -2.0f * t.x * t.z * trxztrxz_inv * trxztrxz_inv;
	float dJ_dx01 = 0.0f;
	float dJ_dx02 = trxztrxz_inv * trxztrxz_inv * (t.x * t.x - t.z * t.z);
	float dJ_dx10 = tr3_inv * trxztrxz_inv * (t.x * t.y * (2.0f * t.x * tr_inv * trxz + t.x * tr * trxz_inv) - t.y * tr * trxz);
	float dJ_dx11 = tr3_inv * (t.x * tr * trxz_inv - 2.0f * t.x * trxz * tr_inv);
	float dJ_dx12 = t.z * t.y * tr3_inv * trxztrxz_inv * (2.0f * t.x * tr_inv * trxz + t.x * tr * trxz_inv);

	float dJ_dy00 = 0.0f;
	float dJ_dy01 = 0.0f;
	float dJ_dy02 = 0.0f;
	float dJ_dy10 = tr3_inv * trxz_inv * (2.0f * t.x * t.y * t.y * tr_inv - t.x * tr);
	float dJ_dy11 = -2.0f * tr3_inv * tr_inv * t.y * trxz;
	float dJ_dy12 = tr3_inv * trxz_inv * (2.0f * t.z * t.y * t.y * tr_inv - t.z * tr);

	float dJ_dz00 = trxztrxz_inv * trxztrxz_inv * (t.x * t.x - t.z * t.z);
	float dJ_dz01 = 0.0f;
	float dJ_dz02 = 2.0f * t.x * t.z * trxztrxz_inv * trxztrxz_inv;
	float dJ_dz10 = t.x * t.y * t.z * trxztrxz_inv * tr3_inv * (tr * trxz_inv + 2.0f * tr_inv * trxz);
	float dJ_dz11 = tr3_inv * t.z * (tr * trxz_inv - 2.0f * trxz * tr_inv);
	float dJ_dz12 = tr3_inv * trxztrxz_inv * (t.y * t.z * t.z *(2.0f * tr_inv * trxz + tr * trxz_inv) - t.y * tr * trxz);
	...
	// float dL_dty = y_grad_mul * -h_y * tz2 * dL_dJ12;
	// float dL_dtz = -h_x * tz2 * dL_dJ00 - h_y * tz2 * dL_dJ11 + (2 * h_x * t.x) * tz3 * dL_dJ02 + (2 * h_y * t.y) * tz3 * dL_dJ12;
	// float dL_dtx = dL_dtx_raw;
	// float dL_dty = dL_dty_raw;
	// float dL_dtz = dL_dtz_raw;
	float dL_dtx = dL_dtx_raw + dL_dlat_from_cov * dgy_dtx;
	float dL_dty = dL_dty_raw + dL_dlat_from_cov * dgy_dty;
	float dL_dtz = dL_dtz_raw + dL_dlat_from_cov * dgy_dtz;
	// Account for inverse depth gradients
	// For omnidirectional: depth = sqrt(tx^2 + ty^2 + tz^2), so invdepth = 1/r
	// d(invdepth)/dt = -t / r^3
	if (dL_dinvdepth) {
		float dL_dinvd = dL_dinvdepth[idx];
		dL_dtx -= dL_dinvd * t.x * tr3_inv;
		dL_dty -= dL_dinvd * t.y * tr3_inv;
		dL_dtz -= dL_dinvd * t.z * tr3_inv;
	}
...
template<int C>
__global__ void preprocessCUDAOmni(
	int P, int D, int M,
	const float3* means,
	const int* radii,
	const float* radius_rad,
	...
	float3* dlat_dt)
{
	...
	float dL_dtx = dL_dmean2D[idx].x * dlon_dt[idx].x + dL_dmean2D[idx].y * dlat_dt[idx].x;
	float dL_dty = dL_dmean2D[idx].x * dlon_dt[idx].y + dL_dmean2D[idx].y * dlat_dt[idx].y;
	float dL_dtz = dL_dmean2D[idx].x * dlon_dt[idx].z + dL_dmean2D[idx].y * dlat_dt[idx].z;
	...
	// if(idx >= 0 && idx <= 15)
	// {
	// 	printf("Idx: %d, dL_dmeans: (%.9f, %.9f, %.9f)\n", idx, dL_dmeans[idx].x, dL_dmeans[idx].y, dL_dmeans[idx].z);
	// }
...
)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			// if (pix_id >= 10000 && pix_id < 10010){
			// 	printf("pix_id: %d, global_id: %d, dL_dmean2D.x: %.9f, dL_dconic2D.x: %.9f, dL_dopacity: %.9f\n", pix_id, global_id, dL_dmean2D[global_id].x, dL_dconic2D[global_id].x, dL_dopacity[global_id]);
			// }
		}
	}
}
...
renderCUDAOmni(
	...
	const float* __restrict__ dL_dpixels,
	const float* __restrict__ dL_invdepths,
	const float* __restrict__ dL_dhppix,
	const float* __restrict__ dL_dhpinvdepths,
	float3* __restrict__ dL_dmean2D,
	float4* __restrict__ dL_dconic2D,
	float* __restrict__ dL_dopacity,
	float* __restrict__ dL_dcolors,
...
)
{
	...
	__shared__ float collected_colors[C * BLOCK_SIZE_3D];
	__shared__ float collected_depths[BLOCK_SIZE_3D];
	...
	if (inside)
	{
		for (int i = 0; i < C; i++){
			// dL_dpixel[i] = dL_dpixels[i * H * W + pix_id];
			dL_dpixel[i] = dL_dhppix[i * N_side * N_side * 12 + pix_id];
			// printf("dL_dhppix[%d]: %.9f\n", i * N_side * N_side * 12 + pix_id, dL_dhppix[i * N_side * N_side * 12 + pix_id]);
		}
		// Read HEALPix depth gradient if output depth gradient is enabled
		if(dL_dinvdepths && dL_dhpinvdepths)
			dL_invdepth = dL_dhpinvdepths[pix_id];
	}
	...
	// const float ddelx_dx = 0.5 * W;
	// const float ddely_dy = 0.5 * H;
	// const float ddelx_dx = 1; //dSphere_dLonlat
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE_3D)
	{
		...
		// and load them in revers order.
		block.sync();
		const int progress = i * BLOCK_SIZE_3D + block.thread_rank();
		if (range.x + progress < range.y)
		{
			const int coll_id = point_list[range.y - progress - 1];
			collected_id[block.thread_rank()] = coll_id;
			...
			collected_conic_opacity[block.thread_rank()] = conic_opacity[coll_id];
			for (int i = 0; i < C; i++)
				collected_colors[i * BLOCK_SIZE_3D + block.thread_rank()] = colors[coll_id * C + i];

			// Load depths if any depth gradient is provided (equirectangular or HEALPix)
			if(dL_dinvdepths)
			collected_depths[block.thread_rank()] = depths[coll_id];
		}
		block.sync();
...
/**
	...
	*/
			...
			greatCircleDistance(lonlat, pix_lonlat, great_circle_rad); //great_circle_rad actually is arc length here
			...
			float dy = great_circle_rad * cosf(bearing);  // East-west component
			float dx = great_circle_rad * sinf(bearing);  // North-south component
/**
	 * ------------------------------- For rad cov calculation  ------------------------------------------
	*/
			// float dx = shortest_angular_difference(pix_lonlat.x, lonlat.x);
			// float dy = lonlat.y - pix_lonlat.y;
			// const float ddelx_dx = fmaxf(1e-6f, cosf(lonlat.y)); //dSphere_dLonlat
			const float ddelx_dx = fmaxf(1e-6f, cosf(pix_lat)); //dSphere_dLonlat
			const float ddely_dy = 1; 
			...
			float dL_dalpha = 0.0f;
			const int global_id = collected_id[j];
			if (global_id < 0 || global_id >= P){
				printf("Error: global_id out of range: %d\n", global_id);
			}
			for (int ch = 0; ch < C; ch++)
			{
				const float c = collected_colors[ch * BLOCK_SIZE_3D + j];
				// Update last color (to be used in the next iteration)
				accum_rec[ch] = last_alpha * last_color[ch] + (1.f - last_alpha) * accum_rec[ch];
				last_color[ch] = c;
			...
			// Helpful reusable temporary variables
			const float dL_dG = con_o.w * dL_dalpha; // G is the value of the Gaussian Funcion
			// α = opacity * G, ∂L/∂G = ∂L/∂α * ∂α/∂G = = dL_dalpha * opacity = = dL_dalpha * con_o.w
			// const float gdx = G * d.x;
			// const float gdy = G * d.y;
			...
			// Update gradients w.r.t. 2D covariance (2x2 matrix, symmetric)
			atomicAdd(&dL_dconic2D[global_id].x, -0.5f * gdx * dx * dL_dG);
			atomicAdd(&dL_dconic2D[global_id].y, -0.5f * gdx * dy * dL_dG);
			atomicAdd(&dL_dconic2D[global_id].w, -0.5f * gdy * dy * dL_dG);
	...
	bool antialiasing)
{
	...
	// "preprocess". When done, loss gradient w.r.t. 3D means has been
	// modified and gradient w.r.t. 3D covariance matrix has been computed.	
	computeCov2DCUDAOmni << <(P + 255) / 256, 256 >> > (
		P,
		means3D,
		radii,
		radius_rad,
