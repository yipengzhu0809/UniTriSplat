...
// shared memory size
#define SX (BX + 10) //this is because we need to load extra 5 pixels on each side for convolution, the convolution kernel size is 11, 5 is the radius
#define SSX (BX + 10)
#define SY (BY + 10)
...
__device__ void load_into_shared(float pixels[SY][SSX], const float *inp, const int CH, const int H, const int W, const int i) {
  ...
  const int cnt = SY * SX; //the total number of pixels to load into shared memory
  ...
  for (int b = 0; b < num_blocks; ++b) {
    ...
    if (tid < cnt) {
      ...
      float one = get_pix_value(inp, batch, i, y - 5, x - 5, CH, H, W); //-5 is because of the padding for convolution, the (0,0) in shared memory corresponds to (-5,-5) in the image
...
__device__ void do_separable_conv_x(float pixels[SY][SSX], float opt[CY][CCX], int H, int W, bool sq = false) {
  ...
  val = 0.0f;
  local_y = block.thread_index().y + BY; //process the second half of the block
  if (local_y < SY) {
    if (sq) {
      val += G_00 * do_sq(pixels[local_y][local_x - 5]);
      val += G_01 * do_sq(pixels[local_y][local_x - 4]);
...
)
{
  ...
  const int pix_x = block.group_index().x * BX + block.thread_index().x; //this thread is responsible for pixel (pix_y, pix_x)
  const int pix_id = pix_y * W + pix_x;
  const int num_pix = H * W;
  const int batch = block.group_index().z;

  // shared memory that will be used to load pixels temporarily
  __shared__ float buf1[SY][SSX]; //__shared__ is block level shared memory
  __shared__ float buf2[SY][SSX];
  __shared__ float buf3[CY][CCX]; 
  // CCX = 32, CY = 42, because the convolution is seperate into vertical and horizontal, for one direciton, we need extra 5 pixels on top and bottom for convolution
  ...
  for (int i = 0; i < CH; ++i) {
    ...
    // calculate mu1
    flush_conv_scratch(buf3); //clear convolution scratchpad
    block.sync();
    ...
    // calculate sigma1_sq, sq is the variance
    ...
    if (pix_x < W && pix_y < H) {
      ...
      if (dm_dmu1) { // m is the ssim value
        dm_dmu1[global_idx] = (
          (mu2 * 2.0f * D) / (A * B)
          -(mu2 * 2.0f * C) / (A * B)
          -(mu1 * 2.0f * C * D) / ( A * A * B)
...
__global__ void fusedssim_backwardCUDA(
  ...
  float* img1,
  float* img2,
  float *dL_dmap, // gradient of loss w.r.t. ssim map
  float *dL_dimg1,
  float* dm_dmu1 = nullptr,
  float* dm_dsigma1_sq = nullptr,
  float* dm_dsigma12 = nullptr
...
)
{
  ...
  fusedssimCUDA<<<grid,block>>>(
    ...
    dm_dmu1.contiguous().data<float>(), //the gradient dSSIM/dmu1, mu1 is the local mean of img1
    dm_dsigma1_sq.contiguous().data<float>(), //the gradient dSSIM/dsigma1_sq, sigma1_sq is the local variance of img1
    dm_dsigma12.contiguous().data<float>() // the gradient dSSIM/dsigma12, sigma12 is the local covariance of img1 and img2
  );
